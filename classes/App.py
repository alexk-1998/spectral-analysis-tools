    ...
    def _straight_line_continuum_removal(self, x: np.array, y_list: list, x_pts: list, y_pts: list) -> list:
        ...
        for y in y_list:
            ...
            for x_pt, y_pt in zip(x_pts, y_pts):
                ...
                box_area = np.trapz(np.ones_like(y_continuum[mask]), x=x_continuum[mask])
                # area underneath the curve
                under_area = np.trapz(y_continuum[mask], x=x_continuum[mask])
                # area between curve and y=1
                area = box_area - under_area
                # TODO: print in the GUI
                print('Curve Analytics:')
                print(f'\tarea: {area:.2f}', area)
